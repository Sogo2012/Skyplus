    ...
    if st.session_state.diseno_completado and st.session_state.resultado_diseno:
        ...
        if not st.session_state.lead_capturado:
            ...
            st.markdown(
                ...
                "El reporte técnico se envía a tu correo."
            )
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            ...
            with st.form("formulario_leads_cta"):
                ...
                enviado = st.form_submit_button(
                    "Ver curva de optimización completa",
                    use_container_width=True,
                    type="primary",
                )
        ...
        # ── Etapa 2 — curva completa ──────────────────────────────────────
        if st.session_state.lead_capturado:

            if not st.session_state.calculo_completado:
                st.success(
                    f"Calculando curva completa para {st.session_state.lead_empresa} "
                    f"({st.session_state.lead_nombre})..."
                )

                barra2      = st.progress(0, text="Iniciando análisis completo...")
                status_box2 = st.empty()

                def actualizar_progreso2(paso, total, mensaje):
                    pct = int(paso / max(total, 1) * 100)
                    barra2.progress(pct, text=mensaje)
                    status_box2.caption(mensaje)

                try:
                    config2 = configurar_proyecto(
                        ancho        = ancho_nave,
                        largo        = largo_nave,
                        altura       = alto_nave,
                        tipo_uso     = tipo_uso,
                    ...
                    sql_base   = st.session_state.resultado_diseno.get("sql_base")
                    resultado2 = calcular_curva_sfr(
                        config2,
                        callback=actualizar_progreso2,
                        sql_base_existente=sql_base,
                    )

                    if resultado2.get("error"):
                        barra2.empty(); status_box2.empty()
                        st.error(f"Error: {resultado2['error']}")
                    else:
                        barra2.progress(100, text="Análisis completado.")
                        status_box2.empty()
                        st.session_state.resultado_motor    = resultado2
            ...
            if st.session_state.calculo_completado and st.session_state.resultado_motor:
                res2 = st.session_state.resultado_motor

                st.divider()
                section_title("Curva de optimización completa")

                render_cards([
                    {"label": "SFR óptimo energético",
                     "value": f"{res2['sfr_opt']}%",
                     "delta": f"{res2['neto_opt']:,.0f} kWh/año",
                     "green": True},
                    {"label": "Ahorro máximo",
                     "value": f"{res2['pct_opt']:.1f}%",
                     "delta": f"Base: {res2['kwh_base']:,.0f} kWh/año"},
                ])
                _fila2 = []
                if res2["sfr_dual"] is not None:
                    idx_d = [r["sfr_pct"] for r in res2["df_curva_raw"]].index(res2["sfr_dual"])
                    _fila2.append({
                        "label": "SFR óptimo dual",
                        "value": f"{res2['sfr_dual']}%",
                        "delta": f"{res2['fc_lux'][idx_d]:.0f} lux — Confort verificado",
                        "green": True,
                    })
                _fila2.append({
                    "label": "Consumo base",
                    "value": f"{res2['kwh_base']:,.0f} kWh/año",
                    "delta": "SFR=0% sin domos",
                })
                render_cards(_fila2)

                st.plotly_chart(fix_figura(res2["figura"]), use_container_width=True)
                ...
                df_show = res2["tabla"].copy()
                df_show["Semáforo"] = df_show["Semáforo"].map(lambda x: semaforo_texto.get(x, x))
                section_title("Tabla de resultados por SFR")
                st.dataframe(df_show, use_container_width=True, hide_index=True)

                st.divider()
                section_title("Recomendación SkyPlus")
                st.markdown(res2["recomendacion"])

                st.markdown(f"""
                <div class="eco-disclaimer">
                    <strong>Nota técnica:</strong> Resultados generados por EnergyPlus 23.2 (DOE).
                    La métrica de confort visual (fc promedio) es una estimación analítica EPW
                    bajo normativa ISO 8995-1 e IES RP-7. Para validación espacial punto por punto,
        ...
        if st.button("Nueva simulación (limpiar resultados)"):
            for k in ["resultado_diseno","resultado_motor","diseno_completado",
                      "calculo_completado","lead_capturado","lead_nombre",
                      "lead_empresa","lead_correo"]:
                st.session_state[k] = False if ("completado" in k or "capturado" in k) else None
                if k in ["lead_nombre","lead_empresa","lead_correo"]:
                    st.session_state[k] = ""
            st.rerun()
