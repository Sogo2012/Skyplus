...
try:
    ...
    from motor.job    import lanzar_simulacion_background
    from motor.sheets import verificar_cuota
    MOTOR_DISPONIBLE = True
except ImportError:
    MOTOR_DISPONIBLE = False
    def lanzar_simulacion_background(*a, **kw): pass
    def verificar_cuota(*a, **kw): return 0, True
...
with tab_analitica:
    ...
    if st.session_state.diseno_completado and st.session_state.resultado_diseno:
        ...
        if st.session_state.lead_capturado and not st.session_state.bg_lanzado:
            ...
            # Lanzar thread background — retorna inmediatamente
            thread_name = lanzar_simulacion_background(_config_bg, _lead_bg, _sql_base)
            st.session_state.bg_lanzado     = True
            st.session_state.bg_thread_name = thread_name
            st.rerun()
